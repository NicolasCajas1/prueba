...
float operacionTransporte() {
    ...
    for (i = 1; i <= 5; i++) {
        ...
        descuento=calcularDescuento(costoSinDescuento);
        //calcula el costo de la ruta sin descuento y el monto del descuento.
    }
    float costoFinal = costoTotal - descuento;
    printf("Subtotal: $%.2f\n", costoTotal);//$%.2f se utiliza para imprimir valores de punto flotante (floats) con dos decimales. 
    printf("Descuento: $%.2f\n", descuento);
